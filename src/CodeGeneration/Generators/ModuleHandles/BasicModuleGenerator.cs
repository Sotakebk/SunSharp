...
public class BasicModuleGenerator : BaseGenerator
{
    ...
    protected virtual void GenerateInterfaceCurveXmlDoc(int i, CurveDescription c, bool write)
    {
        ...
        if (c.Description != null)
        {
            ...
            AppendLine($"/// {string.Join("<br/>", descriptionLines)}");
        }
        AppendLine("/// </para>");
        AppendLine("/// <para>");
        if (write)
        {
            AppendLine($"/// Write to curve {i} of {FriendlyName}.");
        }
        else
        {
            AppendLine($"/// Read from curve {i} of {FriendlyName}.");
        }
        AppendLine("/// </para>");
        AppendLine($"/// The curve contains {c.Size} values in range of {c.MinValue} to {c.MaxValue}.");
        AppendLine("/// </summary>");
    }
