...
{
    public partial class SunVoxLibNative : ISunVoxLib
    {
        private readonly ISunVoxLibC _lib;

        public SunVoxLibNative(ISunVoxLibC nativeLibrary)
        {
            _lib = nativeLibrary;
        }
        ...
        /// <inheritdoc/>
        public int GetSongLengthInFrames(int slotId)
        {
        ...
        private bool AudioCallbackInternal<TOut, TIn>(TOut[] outputBuffer, AudioChannels outputChannels, TIn[] inputBuffer, AudioChannels inputChannels, int latency, uint outTime,
            int inputType)
        {
            ...
            try
            {
                ret = _lib.sv_audio_callback2(outPtr, outputFrames, latency, outTime, inputType, (int)inputChannels, inPtr);
            }
            finally
            {
                inHandle.Free();
